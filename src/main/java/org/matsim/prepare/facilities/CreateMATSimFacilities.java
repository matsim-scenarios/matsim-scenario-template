...
//		ShpOptions.Index zones = zoneShp != null ? zoneShp.createIndex(OpenGunmaScenario.CRS, "SCHLUESSEL") : null;
		...
		for (Holder h : data) {
			...
			// TODO: added link Id, don't know why that wasn't done previously -jr
			ActivityFacility facility = f.createActivityFacility(id, CoordUtils.round(new Coord(x.getAsDouble(), y.getAsDouble())), h.linkId);
...
//			}
	...
	private Holder processFeature(SimpleFeature ft, Network network) {
		...
		Map<Id<Link>, Long> map = links.stream()
			// TODO: We will ignore this for now, since we our links don't have a type
