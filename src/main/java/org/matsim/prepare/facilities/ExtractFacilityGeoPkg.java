...
public class ExtractFacilityGeoPkg implements MATSimAppCommand {
	...
	@CommandLine.Mixin
	private CrsOptions crs = new CrsOptions(TransformationFactory.EPSG4326, OpenGunmaScenario.CRS);
	...
	public Integer call() throws Exception {
		...
		if(crs.getInputCRS().equals(TransformationFactory.EPSG4326)){
			inputCrs = CRS.decode(crs.getInputCRS(), true);
		} else {
			inputCrs = CRS.decode(crs.getInputCRS(), true);
		}
		transform = CRS.findMathTransform(inputCrs, CRS.decode(crs.getTargetCRS(), true), true);
